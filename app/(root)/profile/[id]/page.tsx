...
import {
  ...
  getUsersAnswersAction,
  getUserStatsAction,
  getUserTopTagsAction,
...
const Profile = async ({ params, searchParams }: RouteParams) => {
  ...
  // searchParams --> ?id=1&page=1&pageSize=10
  ...
  const { success, data, error } = await getUserByIdAction({ userId: id });
  ...
  const { data: userStats } = await getUserStatsAction({ userId: id });
      ...
      <section className="mt-10 flex gap-10">
        <Tabs defaultValue="top-posts" className="flex-[2]">
          <TabsList className="background-light800_dark400 min-h-[42px] p-1">
            <TabsTrigger value="top-posts" className="tab z-10 cursor-pointer">
              Top Posts
            </TabsTrigger>
            <TabsTrigger value="answers" className="tab z-10 cursor-pointer">
              Answers
            </TabsTrigger>
          </TabsList>
